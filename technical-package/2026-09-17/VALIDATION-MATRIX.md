...
| STAT-01 | New hole has **no recorded performance stats**. Advancing or finishing cannot turn untouched defaults into FIR / GIR / putt facts. Course par may be present as metadata only. | Create or advance onto a new hole. Inspect hole record + UI before any +/− / drive input. Then NEXT/lock/finish **without** entering strokes, putts, or drive. Inspect locked hole, derived totals (`slab_round_fwy_hits`, `slab_round_gir_hits`, `slab_round_putts`), and serialized payload. | New hole: strokes/putts/drive unset or 0-unrecorded — not par, not 2, not fairway-hit. After untouched advance/finish: no captured strokes, putts, or drive_result; no FIR hit; no GIR fact; no two-putt fact. Par may remain as course metadata. A payload must not emit an untouched `H`/`fairway` or fabricated putt/stroke as captured data. | None. PR #1 `slab_hole_apply_defaults` prefills par / 2 / `SLAB_FWY_H` and will **fail** this gate until firmware changes. | **TBD** / **FAIL vs current firmware** |
| STAT-02 | GIR / FIR / putt totals use only golfer-entered values | Play a hole with explicit Slab entry; compare to an untouched hole that was illegally advanced (if the firmware still allows it) | Entered hole may contribute GIR/FIR/putts. Untouched hole must contribute none. Derived GIR must not run on uncaptured zeros or prefills. | None. Current `slab_hole_gir` / `slab_round_fwy_hits` run on locked prefills. | **TBD** / **FAIL vs current firmware** |
