...
| Buttons | 5× Omron **B3W-1000** to GND, `INPUT_PULLUP` | Same family or sealed-cap equivalent after IP55 DFM | + D2, − D4, PUTT D5 (**ignored** by sequential lock), MODE D0, NEXT D9; debounce **25 ms** | P1 switch **selected**. Cap/seal **TBD**. |
...
| Firmware | PR #1 `e7f7ad79…`: UI SM, derived GIR, persist API, BLE advertise **stub** + JSON builder | Same product SM + real GATT + signed DFU | See `FIRMWARE-STATUS.md` | Demo boot `LOCAL EXAMPLE`. Inactive/start/course-card **not implemented**. OTA **NOT IMPLEMENTED**. |
...
| Magnet | K&J **DC6TP-N52** in bottom pocket (Ø19.2 × 9.6 CAD pocket) | Same or advised equivalent | Mechanical only; **RF keep-out not modelled** | Catalog pull 13.12 lb Case-1 ≠ cart test |
...
GIR: `slab_hole_gir` = `(strokes − putts) ≤ (par − 2)`. Never prompted.
