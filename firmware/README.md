...
Provision the filesystem separately on fresh boards. Normal startup only mounts it; it never invokes the SDK auto-format path. A mount failure shows a storage error and preserves the medium for service.
